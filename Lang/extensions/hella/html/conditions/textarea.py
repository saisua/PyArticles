...
class textarea(Condition, BaseTag):
	def __init__(self, next_blocks: List[Block] | None = None, *args, **kwargs) -> None:
		BaseTag.__init__(self, 'textarea', *args, next_blocks=next_blocks, **kwargs)
	...
	def trigger(self, action: Action) -> Any:
		...
		self._kwargs['py-input'] = f"{action.name}"
		Condition.trigger(self, action)

	def on_input(self, action: Action):
		self.trigger(action)